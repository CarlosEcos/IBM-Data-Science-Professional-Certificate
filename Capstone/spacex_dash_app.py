...
spacex_df["count"] = [1 for i in range(spacex_df.shape[0])]
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        title='Total Sucess launches by site {}'.format(entered_site))
        return fig
...
def get_pie_chart(entered_site, range):
    mask = (spacex_df["Payload Mass (kg)"] >= range[0]) & (spacex_df["Payload Mass (kg)"] <= range[1])
